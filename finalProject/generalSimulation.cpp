...
struct SimulationNoise{
    ...
    SimulationNoise() : omega_dist(0, 0.1), delta_dist(0, 0.01) { // adjust stdev and mean if needed HERE
...
int main() {
    ...
    std::string filename = "simulation_11.csv";
    double res = 0.01;

    // 1. Initialize superposition on the first qubit
    simulate_added_gate(qs, {0}, "H", PI/2, PI/2, res, filename);

    // 2. Cascade CNOTs (MCX with 2 targets) to entangle all four qubits
    simulate_added_gate(qs, {0, 1}, "MCX", PI, 0, res, filename);
    simulate_added_gate(qs, {1, 2}, "MCX", PI, 0, res, filename);
    simulate_added_gate(qs, {2, 3}, "MCX", PI, 0, res, filename);

    // 3. Apply a symmetric 'X' gate layer to the middle qubits
    simulate_added_gate(qs, {1}, "X", PI, 0, res, filename);
    simulate_added_gate(qs, {2}, "X", PI, 0, res, filename);

    // 4. Final Hadamard wash to redistribute amplitudes
    simulate_added_gate(qs, {0, 1, 2, 3}, "H", PI/2, PI/2, res, filename);
